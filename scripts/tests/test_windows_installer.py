...
def test_release_bundle_has_double_click_launcher_and_user_instructions():
    ...
    assert "nuoyan-windows-standard-assets-2.3.2.zip" in launcher
...
def test_windows_validation_prompt_targets_the_github_release_bundle():
    ...
    assert "nuoyan-windows-offline-installer-2.3.2.zip" in prompt
    assert "INSTALL_NUOYAN.cmd" in prompt
    assert "nuoyan-skill-v2 源码目录" in prompt
    assert "nuoyan-skill-v2-2.3.2 源码目录" not in prompt
...
def test_windows_users_are_directed_to_the_github_offline_installer():
    ...
    assert "nuoyan-windows-offline-installer-2.3.2.zip" in readme
    assert "nuoyan-windows-offline-installer-2.3.2.zip" in guide
...
def test_windows_environment_release_has_distinct_version():
    ...
    assert project["project"]["version"] == "2.3.2"
    assert "v2.3.2" in WORKFLOW_VERSION
    assert "V2.3.2" in readme
