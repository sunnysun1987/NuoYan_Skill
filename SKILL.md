...
- HTML 主报告默认采用“研发筛选版”工作台形态：顶部页签为“项目分析 / 研发阅读入口 / 核心必读文献 / 证据卡筛选器”。不同页签应提供各自需要的目录或筛选器，不得在所有页签中固定保留全局证据筛选器；证据卡筛选器只在“证据卡筛选器”页签内展示。证据卡必须展示优先级、研发阶段、样本类型、检测平台/方法、参照标准、应用场景、目标人群、性能参数、组合标志物、地域/人群、证据类型、关键摘录和待复核点。
...
6. 生成报告：输出 V2.1 标准交付目录中的 `00_立项调研综合报告.html`，报告必须采用研发筛选版工作台，含项目分析、研发阅读入口、核心必读文献和证据卡筛选器；证据地图、缺口任务、关键证据、文献、竞品、标准、专利和指标事实应在这些入口中以业务可读方式呈现。
