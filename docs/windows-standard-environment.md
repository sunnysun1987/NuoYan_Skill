...
仓库不提交数百 MB 的二进制运行时。`v2.3.2` 标签触发 GitHub Actions 在 Windows 环境构建和发布标准资产包、源码包、校验文件与完整离线安装包。企业也可以在批准的 Windows 构建机运行 `packaging\windows\build-assets.ps1`，通过内网镜像、U 盘、企业文件共享或软件分发平台交付相同资产。
...
打开 [GitHub Releases 最新版本](https://github.com/sunnysun1987/NuoYan_Skill/releases/latest)，下载 `nuoyan-windows-offline-installer-2.3.2.zip`。完整解压后双击 `INSTALL_NUOYAN.cmd`，等待安装和体检完成。安装包内已包含 Skill 源码、Python 3.13、Python 依赖、Playwright Chromium 和 Argos 英中模型，不要求电脑预装 Git、Python、Node 或 Java。
...
构建完成后，必须审阅 release manifest、实际文件大小、SHA-256 和 `THIRD_PARTY_LICENSES.txt`，再把 `nuoyan-windows-standard-assets-2.3.2.zip` 与 Skill 源码包一起交付。模板 manifest 的 `release_ready=false` 和全零哈希不能用于正式安装。
...
| `windows_install_state` | 未找到有效 manifest 版本、资产来源或安装记录 | IT 使用 V2.3.2 标准包重新安装并保留 JSON |
